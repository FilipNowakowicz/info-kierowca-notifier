...
class TriggerAutoRefreshBackoffTests(unittest.TestCase):
    def setUp(self):
        ...
        self.logger = Mock()
        self.path_patch = patch.object(notifier, "RELOGIN_BACKOFF_FILE", self.state_path)
        self.path_patch.start()
        self.addCleanup(self.path_patch.stop)
        self.log_patch = patch.object(
            notifier, "AUTO_REFRESH_LOG_FILE", Path(self.directory.name) / "auto-refresh.log"
        )
    ...
    def test_automatic_attempt_stops_during_persisted_cooldown(self):
        ...
        with patch("info_kierowca_notifier.notifier.chrome.chrome_available") as available:
    ...
    def test_manual_attempt_bypasses_persisted_cooldown(self):
        RetryBackoff(self.state_path).record_failure("authentication_timeout")
        with patch("info_kierowca_notifier.notifier.chrome.chrome_available", return_value=False):
    ...
    def test_automatic_launch_marks_helper_and_records_launch_failure(self):
        with patch("info_kierowca_notifier.notifier.chrome.chrome_available", return_value=True), patch.object(
